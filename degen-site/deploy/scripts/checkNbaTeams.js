...
(async () => {
  try {
    ...
    console.log(`Total NBA teams: ${teams.length}\n`);
    ...
    Object.keys(bySeason).forEach(season => {
      ...
      if (bySeason[season].length <= 5) {
        bySeason[season].forEach(name => console.log(`  - ${name}`));
      } else {
        bySeason[season].slice(0, 3).forEach(name => console.log(`  - ${name}`));
        console.log(`  ... and ${bySeason[season].length - 3} more`);
      }
      console.log('');
    });
  } catch (error) {
