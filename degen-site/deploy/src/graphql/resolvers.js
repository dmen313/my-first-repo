...
const resolvers = {
  Query: {
    ...
    getDraftPick: (_, { id }) => {
      return dataStore.getDraftPick(id);
    },
  ...
  Mutation: {
    ...
    // Draft mutations
    createDraftPick: (_, { input }) => {
      return dataStore.createDraftPick(input);
    },

    updateDraftPick: (_, { input }) => {
      const { id, ...updateData } = input;
      const updatedPick = dataStore.updateDraftPick(id, updateData);
      if (!updatedPick) {
    ...
    deleteDraftPick: (_, { id }) => {
      return dataStore.deleteDraftPick(id);
    },

    initializeDraft: (_, { league, season, owners }) => {
      return dataStore.initializeDraft(league, season, owners);
    },

    reorderDraftPicks: (_, { league, season, owners }) => {
      return dataStore.reorderDraftPicks(league, season, owners);
    },
